...
import { generateQuizQuestions, listModels } from "../../src/providers/deepseek";
...
describe("deepseek.generateQuizQuestions", () => {
  ...
  it("calls the correct DeepSeek endpoint with Bearer auth", async () => {
...
describe("deepseek.listModels", () => {
  ...
  it("calls the DeepSeek models endpoint with Bearer auth", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
